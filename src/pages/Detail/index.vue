            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr, index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  :key="item.id"
                  @click="changeCur(attr, item)"
                >
                  <!-- @click="changeActive(value, attr)" -->
...
export default {
  ...
  methods: {
    ...
    // 因为这里关乎商品信息上有一个属性是ischeckde,所以用排它，数组里面只有一个元素
    // 是被选中的
    changeCur(attr, item) {
      attr.spuSaleAttrValueList.forEach((m) => {
        m.isChecked = 0;
      });
      item.isChecked = 1;
    },
    async addCart() {
      try {
        ...
        await this.$store.dispatch("getShopCart", data);
        // 在跳转购物添加成功的页面时，把产品的信息带过去，就可以省的去发请求
        // 路由传参比较适合传递简单的参数，发杂的对象在界面不好看，考虑采取本地存储
        // 注意格式的转换
